...
class Profile extends Dbh
{
    ...
    protected function changesAddress($name, $district, $box, $email, $phone, $mpamba)
    {
        $sql1 = "UPDATE company_address SET `company_name` = ?, `postal_box` = ?, `district` = ?, `email` = ?, `phone` = ?, `mpamba` = ?";
        $stmt1 = $this->connect()->prepare($sql1);
        $result = $stmt1->execute([$name, $box, $district, $email, $phone, $mpamba]);
        if ($result) {
